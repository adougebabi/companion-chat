...
// CapabilityType describes the policy/audit class of a capability.
...
// CapabilitySurface identifies a provider-facing catalog.
...
// CapabilityFailurePolicy controls visible-settlement behavior.
...
// ContextSlot is a typed request for execution context.
...
func (invocation CapabilityInvocation) Validate(definition CapabilityDefinition) error {
	...
	if err := ValidateCapabilitySchemaValue(object, definition.InputSchema); err != nil {
		return fmt.Errorf("%w: %v", ErrInvalidArguments, err)
	}
...
func (runtime *CapabilityRuntime) Execute(ctx context.Context, invocation CapabilityInvocation) (CapabilityResult, error) {
	...
	if result.CallID != "" && result.CallID != invocation.CallID {
		return FailedCapabilityResult(invocation, "execution_failed", false), fmt.Errorf("%w: capability result identity mismatch", ErrCapabilityExecution)
	}
	if result.CallID == "" {
	...
	if executeErr != nil {
		code, retryable := CapabilityErrorInfo(executeErr, "capability_execution_failed", true)
	...
	if result.Status == "completed" {
		if err := definition.ValidateOutput(result.Output); err != nil {
			result = FailedCapabilityResultDetail(invocation, "capability_output_invalid", false, err.Error())
			runtime.log(ctx, invocation, definition, result, started, err)
