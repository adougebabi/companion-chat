...
func TestProviderPromptInstructionsStayCompactAndPreserveContracts(t *testing.T) {
	checks := []struct {
		...
		{name: "conversation", value: conversationAssessmentInstruction, max: 1800, must: []string{"conversation.reply", "affect_event", "claims", "response_plan", "no_op", "visible_text", "kind", "content", "evidence_refs", "current_message.content"}},
