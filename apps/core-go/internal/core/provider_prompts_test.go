...
func TestProviderPromptInstructionsStayCompactAndPreserveContracts(t *testing.T) {
	checks := []struct {
		...
		{name: "conversation", value: conversationAssessmentInstruction, max: 1500, must: []string{"conversation.reply", "claims", "response_plan", "no_op", "visible_text", "kind", "content", "evidence_refs", "current_message.content"}},
