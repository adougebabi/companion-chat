...
func (a *App) ProcessDailyReview(ctx context.Context, fluctlightID, localDate string) (map[string]any, error) {
	...
	completion, err := a.Provider.StructuredAssembledWithToolsSchema(providerCtx, "cognitive_assessment", assembly.Messages, definitions, "daily_review_response", schema, structuredThinkingEnabledForSchema("daily_review_response"))
	if err != nil {
	...
	if err != nil {
		...
		decision["capability_results"] = capabilityResultValues(mergeCapabilityResultsFromValue(decision["capability_results"], capabilityBatchFailures(err)))
	}
