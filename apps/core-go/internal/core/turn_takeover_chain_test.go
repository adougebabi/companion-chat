...
func TestTakeoverChainStaticGuards(t *testing.T) {
	...
	if count := strings.Count(takeoverSource, "StructuredAssembledJudgement("); count != 1 {
		t.Fatalf("the Judge must be invoked from exactly one place, found %d", count)
	}
	if count := strings.Count(takeoverSource, ".StructuredAssembledWithToolsSchema("); count != 1 {
	...
	if judgeIndex := strings.Index(takeoverSource, "StructuredAssembledJudgement("); judgeIndex < 0 || judgeIndex < mutexIndex {
		t.Fatal("the QUERY mutual exclusion must be decided before the Judge is invoked")
	}
	if takeoverIndex := strings.Index(takeoverSource, ".StructuredAssembledWithToolsSchema("); takeoverIndex < 0 || takeoverIndex < mutexIndex {
