...
const (
	capabilityConversationPolicyInstruction = "对当前 Actor 消息完成一次正式 Agent 决策。需要外部事实时调用已注册查询 Tool，并依据真实 Tool result 继续；需要状态改变或发布时调用对应写 Tool，并依据其 completed、accepted、rejected 或 failed 结果自行调整。可以连续多轮或同轮调用多个 Tool，不输出正文中的工具协议，也不在最终结构中复制 tool_calls。最终只返回稳定的认知与回复契约；若已调用 conversation.reply，最终 visible_text 仅用于语义记录，调用方不会重复发布；若未调用 conversation.reply，必须提供非空 visible_text，由统一发布服务提交一次。状态改变必须由真实 Tool Call 承担，不解析 prose、关键词或 visible_text 来替代能力调用，不伪造数据库、工作流、证据或渲染结果，不输出 hidden reasoning。claims 只保留有 evidence_refs 的 kind/content/confidence；response_plan.profile_id 与 output_preference_decision 按 schema 返回。"
	capabilityWakeUpPolicyInstruction       = "评估一次内部 wake-up，并在同一正式 Agent 循环中调用所需 Tool。每个 Tool result 都是真实已提交或真实失败的业务结果；可继续查询、调整或选择其他动作。无明确动作返回 no_op。最终结构只描述认知结果，不复制 tool_calls，不把 accepted 冒充 completed，不从时间、关键词或提示词推导动作，不输出 hidden reasoning。"
