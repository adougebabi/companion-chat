...
func TestConversationAgentMayInstallCanonicalToolOutsideDefaultCatalog(t *testing.T) {
	registry := mustCapabilityRegistry(builtinCapabilities(nil)...)
	...
	if err := validateADKCapabilityDefinitions(app, []CapabilityDefinition{definition}, CapabilitySurfaceConversation); err != nil {
		t.Fatalf("canonical Tool rejected by execution surface: %v", err)
	}
}
