...
CREATE TABLE IF NOT EXISTS public.relationship_governance (id varchar(128) PRIMARY KEY, relationship_id varchar(128) NOT NULL, action varchar(32) NOT NULL, actor_id varchar(128) NOT NULL, reason text, created_at timestamptz NOT NULL DEFAULT now());
...
CREATE TABLE IF NOT EXISTS public.media_intents (id varchar(128) PRIMARY KEY, owner_fluctlight_id varchar(128) NOT NULL, kind varchar(32) NOT NULL, mime_type varchar(128) NOT NULL, prompt text NOT NULL, provider_request_id varchar(128) NOT NULL UNIQUE, provider_job_id varchar(256) UNIQUE, workflow_id varchar(128) NOT NULL UNIQUE, conversation_id varchar(128), moment_id varchar(128), status varchar(32) NOT NULL DEFAULT 'pending', revision integer NOT NULL DEFAULT 0, created_at timestamptz NOT NULL DEFAULT now());
...
CREATE TABLE IF NOT EXISTS public.life_presence_overlays (id varchar(128) PRIMARY KEY, fluctlight_id varchar(128) NOT NULL, actor_id varchar(128) NOT NULL, scene varchar(512), activity varchar(512), location varchar(512), expires_at timestamptz, created_at timestamptz NOT NULL DEFAULT now());
CREATE TABLE IF NOT EXISTS public.moment_unread_markers (id varchar(128) PRIMARY KEY, owner_fluctlight_id varchar(128) NOT NULL, actor_id varchar(128) NOT NULL, moment_id varchar(128) NOT NULL, created_at timestamptz NOT NULL DEFAULT now());
