...
function sectionFromLocation(view: WorkspaceView): WorkspaceSection | null {
  ...
  if (view === "diagnostics" && ["lifecycle", "model-runs", "media-prompts", "events", "workflows"].includes(section)) return section as DiagnosticsSection;
  return null;
}
...
const activeDiagnosticsSection = computed<DiagnosticsSection | null>(() => view.value === "diagnostics" && activeSection.value && ["lifecycle", "model-runs", "media-prompts", "events", "workflows"].includes(activeSection.value) ? activeSection.value as DiagnosticsSection : null);
