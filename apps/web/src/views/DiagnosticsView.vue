...
const mediaPromptEvents = computed(() => controlCenter.diagnostics.filter((event) => event.eventType === "media.comfyui.prompt_submitted"));
...
function mediaPromptText(value: unknown): string {
  if (!value || typeof value !== "object" || Array.isArray(value)) return "";
...
<template>
  <section class="page diagnostics-page" aria-labelledby="diagnostics-title">
    ...
    <template v-else>
      ...
      <div v-else-if="(currentSection === 'model-runs' && !controlCenter.diagnosticModelRuns.length && !mediaPromptEvents.length) || (currentSection === 'events' && !controlCenter.diagnostics.length)" class="empty-panel compact"><h2>暂无当前诊断记录</h2><p>该主题暂时没有可展示的脱敏记录。</p></div>
      <div v-else class="diagnostics-groups">
      <Accordion type="single" :default-value="currentSection" class="diagnostics-accordion">
        <AccordionItem v-if="currentSection === 'model-runs' && (controlCenter.diagnosticModelRuns.length || mediaPromptEvents.length)" value="model-runs" class="diagnostic-group diagnostics-drawer">
          <AccordionTrigger class="diagnostics-drawer-summary section-heading"><div><p class="eyebrow">MODEL RUNS</p><h2>模型运行<small v-if="queueSummary" class="queue-summary"> · 队列 {{ queueSummary }}</small></h2></div><Badge class="count-pill" variant="secondary">{{ controlCenter.diagnosticModelRuns.length + mediaPromptEvents.length }}</Badge></AccordionTrigger>
          <AccordionContent><div class="diagnostic-drawer-body"><article v-for="event in mediaPromptEvents" :key="event.id" class="diagnostic-row diagnostic-prompt-event"><div class="diagnostic-meta"><strong>ComfyUI 生图提示词</strong><Badge class="status-pill run-completed" variant="secondary">已提交</Badge><small>{{ event.correlationId }}</small></div><p class="diagnostic-generated-prompt"><strong>实际提交的提示词：</strong>{{ mediaPromptText(event.payload) }}</p><details><summary>查看提交信息</summary><pre>{{ pretty(event.payload) }}</pre></details></article><article v-for="run in controlCenter.diagnosticModelRuns" :key="run.id" class="diagnostic-row"><div class="diagnostic-meta"><strong>{{ scenarioLabel(run.scenario || run.role) }}</strong><Badge class="status-pill" :class="statusClass(run.status)" variant="secondary">{{ statusLabel(run.status) }}</Badge><small>绑定：{{ bindingLabel(run.bindingRole || run.role) }} · {{ run.modelId }}<template v-if="run.priority"> · 优先级 {{ run.priority }}</template><template v-if="run.queuePosition"> · 队列第 {{ run.queuePosition }}</template> · <time class="diagnostic-time" :datetime="run.createdAt">{{ formatRunTime(run.createdAt) }}</time><template v-if="run.queuedAt && run.queuedAt !== run.createdAt"> · 排队 {{ formatRunTime(run.queuedAt) }}</template><template v-if="run.startedAt"> · 开始 {{ formatRunTime(run.startedAt) }}</template><template v-if="run.completedAt"> · 结束 {{ formatRunTime(run.completedAt) }}</template> · {{ run.correlationId }}</small></div><p v-if="run.errorCode" class="diagnostic-error"><strong>失败原因：</strong>{{ run.errorCode }}</p><p v-if="run.role === 'media_prompt' && responseText(run.response)" class="diagnostic-generated-prompt"><strong>最终生图提示词：</strong>{{ responseText(run.response) }}</p><details><summary>查看 Prompt</summary><pre>{{ pretty(run.prompt) }}</pre></details><details v-if="run.response"><summary>查看 Response</summary><pre>{{ pretty(run.response) }}</pre></details></article></div></AccordionContent>
        </AccordionItem>
