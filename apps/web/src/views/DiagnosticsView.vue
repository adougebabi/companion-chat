...
<template>
  <section class="page diagnostics-page" aria-labelledby="diagnostics-title">
    <header class="page-header">
      <div><Button class="back-link" variant="ghost" type="button" @click="emit('back')">← 返回设置</Button><p class="eyebrow">SYSTEM OBSERVABILITY</p><h1 id="diagnostics-title">诊断中心</h1><p class="page-lede">只显示经过脱敏的模型运行、系统事件和工作流状态。</p></div>
      <div class="header-actions"><Button class="secondary-button" variant="outline" type="button" :disabled="refreshing || controlCenter.loading || controlCenter.saving" @click="refresh">{{ refreshing || controlCenter.loading ? "刷新中..." : "刷新" }}</Button><Button class="secondary-button" variant="outline" type="button" :disabled="controlCenter.saving" @click="controlCenter.exportDiagnostics">导出</Button><Button class="secondary-button" variant="outline" type="button" :disabled="controlCenter.saving" @click="controlCenter.clearDiagnostics">清空</Button></div>
    </header>
    ...
    <div v-else class="diagnostics-groups">
      <section v-if="controlCenter.diagnosticModelRuns.length" class="diagnostic-group"><div class="section-heading"><div><p class="eyebrow">MODEL RUNS</p><h2>模型运行</h2></div><Badge class="count-pill" variant="secondary">{{ controlCenter.diagnosticModelRuns.length }}</Badge></div><article v-for="run in controlCenter.diagnosticModelRuns" :key="run.id" class="diagnostic-row"><div class="diagnostic-meta"><strong>{{ roleLabel(run.role) }}</strong><Badge class="status-pill" variant="secondary">{{ run.status }}</Badge><small>{{ run.modelId }} · {{ run.correlationId }}</small></div><p v-if="run.errorCode" class="diagnostic-error"><strong>失败原因：</strong>{{ run.errorCode }}</p><details open><summary>查看 Prompt</summary><pre>{{ pretty(run.prompt) }}</pre></details><details v-if="run.response" open><summary>查看 Response</summary><pre>{{ pretty(run.response) }}</pre></details></article></section>
      <section v-if="controlCenter.diagnostics.length" class="diagnostic-group"><div class="section-heading"><div><p class="eyebrow">EVENTS</p><h2>系统事件</h2></div><Badge class="count-pill" variant="secondary">{{ controlCenter.diagnostics.length }}</Badge></div><article v-for="event in controlCenter.diagnostics" :key="event.id" class="diagnostic-row"><div class="diagnostic-meta"><strong>{{ event.eventType }}</strong><Badge class="status-pill" variant="secondary">{{ event.severity }}</Badge><small>{{ event.correlationId }}</small></div><pre>{{ pretty(event.payload) }}</pre></article></section>
    </div>
