...
<template>
  <section class="page moments-page" aria-labelledby="moments-title">
    <header class="page-header">
      ...
      <span v-if="controlCenter.moments.length" class="count-pill">{{ controlCenter.moments.length }} 条</span>
    </header>
    <div class="feed-toolbar" role="group" aria-label="动态范围">
      <button class="filter-chip" :class="{ selected: controlCenter.momentsScope === 'global' }" type="button" :aria-pressed="controlCenter.momentsScope === 'global'" @click="controlCenter.momentsScope = 'global'">全部动态</button>
      <button class="filter-chip" :class="{ selected: controlCenter.momentsScope === 'fluctlight' }" type="button" :aria-pressed="controlCenter.momentsScope === 'fluctlight'" :disabled="!store.selectedFluctlight" @click="controlCenter.momentsScope = 'fluctlight'">{{ store.selectedFluctlightName ?? "当前实例" }}</button>
      <label class="checkbox-chip"><input v-model="controlCenter.includeHiddenMoments" type="checkbox" />显示已隐藏</label>
    </div>
    ...
    <section v-else class="moments-feed" aria-label="动态列表">
      <article v-for="moment in controlCenter.moments" :key="moment.id" class="moment-card" :class="{ hidden: moment.status === 'hidden' }">
        ...
        <div class="moment-actions"><button class="secondary-button" type="button" @click="controlCenter.reactToMoment(moment.id, store.fluctlightId)">{{ moment.viewer_reaction ? "已回应" : "回应" }}</button><button class="secondary-button" type="button" @click="controlCenter.setMomentStatus(moment.id, moment.status === 'hidden' ? 'restore' : 'hide', store.fluctlightId)">{{ moment.status === "hidden" ? "恢复" : "隐藏" }}</button><form @submit.prevent="controlCenter.commentOnMoment(moment.id, store.fluctlightId)"><label class="sr-only" :for="`comment-${moment.id}`">评论动态</label><input :id="`comment-${moment.id}`" v-model="controlCenter.momentDrafts[moment.id]" placeholder="写评论..." :disabled="moment.status === 'hidden'" /><button class="secondary-button" type="submit" :disabled="moment.status === 'hidden' || !controlCenter.momentDrafts[moment.id]?.trim()">评论</button></form></div>
      </article>
    </section>
  </section>
</template>
