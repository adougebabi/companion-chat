...
: "${API_ORIGIN:?set API_ORIGIN to the public Web/API origin}"
...
curl --fail-with-body --max-time 30 -sS -b "$COOKIE_JAR" \
  "$API_ORIGIN/api/fluctlights/$MOMENT_FLUCTLIGHT_ID/moments" >"$moment_json"
curl --fail-with-body --max-time 30 -sS -b "$COOKIE_JAR" \
  "$API_ORIGIN/api/fluctlights/$MOMENT_FLUCTLIGHT_ID/autonomy-actions" >"$action_json"
...
curl --fail-with-body --max-time 30 -sS -b "$COOKIE_JAR" \
  "$API_ORIGIN/api/fluctlights/$PROACTIVE_FLUCTLIGHT_ID/conversation" >"$conversation_json"
curl --fail-with-body --max-time 30 -sS -b "$COOKIE_JAR" \
  "$API_ORIGIN/api/fluctlights/$PROACTIVE_FLUCTLIGHT_ID/autonomy-actions" >"$action_json"
