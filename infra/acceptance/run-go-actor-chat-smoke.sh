...
api_port=$("${compose[@]}" port web 80 | sed -E 's/.*:([0-9]+)$/\1/')
...
session_headers=$(curl -sS -D - -o "$session_body" -H "Origin: $trusted_origin" "http://127.0.0.1:$api_port/auth/session")
csrf=$(printf '%s\n' "$session_headers" | awk -F'[=;]' 'tolower($1) ~ /set-cookie: fluctlight_csrf/ {print $2; exit}')
test -n "$csrf"
setup_headers=$(curl -sS -D - -o "$setup_body" -c "$cookie_file" -b "fluctlight_csrf=$csrf" -H "Origin: $trusted_origin" -H "X-CSRF-Token: $csrf" -H "Cookie: fluctlight_csrf=$csrf" -H "Content-Type: application/json" --data "{\"setupToken\":\"$token\",\"password\":\"fluctlight-actor-smoke-password\"}" "http://127.0.0.1:$api_port/auth/setup")
...
create_fluctlight() {
  curl -sS -f -H "Origin: $trusted_origin" -H "X-CSRF-Token: $csrf" -H "Cookie: $cookie" -H "Content-Type: application/json" --data "{\"name\":\"$1\"}" "http://127.0.0.1:$api_port/api/fluctlights"
}
...
curl -sS -o "$turn_body" -H "Origin: $trusted_origin" -H "X-CSRF-Token: $csrf" -H "Cookie: $cookie" -H "Accept: application/x-ndjson" -H "Content-Type: application/json" --data "{\"fluctlightId\":\"$fluctlight_one\",\"senderActorId\":\"$fluctlight_two\",\"text\":\"由副摇光发言\",\"idempotencyKey\":\"actor-smoke-turn-1\"}" "http://127.0.0.1:$api_port/api/conversations/$conversation_id/turn" || true
