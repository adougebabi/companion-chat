...
export function createDailyPlanRepository({database} = {}) {
    ...
    function read({personaId, id, at} = {}) {
        ...
        if (!row) return null;
        const timeline = db.prepare(`SELECT * FROM companion_timeline_slots WHERE persona_id = ? AND plan_date = ? ORDER BY starts_at, id`).all(owner, row.plan_date).map(slot => ({...slot, slotId: slot.id, slotKey: slot.slot_key}));
