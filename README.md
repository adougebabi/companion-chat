...
默认不提供调试入口，也不会注册调试 API。仅在本地开发时设置 `COMPANION_DEBUG_INSPECTOR=1`，聊天页才会出现开发检查器；它按当前选中人格显示经过截断和凭据脱敏的提示词组合、最近请求/响应、事件和耐久作业。媒体任务会优先显示最终送入 provider 的提示词；h3 视频任务还会显示真实阶段、尝试次数、耗时、h3 明确输出的百分比（没有则显示未报告）和最新一条经过路径/凭据脱敏的本地输出。检查器中的“测试媒体作业”会实际入队，因此不应在部署环境中开启该开关。
