...
默认不提供调试入口，也不会注册调试 API。仅在本地开发时设置 `COMPANION_DEBUG_INSPECTOR=1`，聊天页才会出现开发检查器；它按当前选中人格显示经过截断和凭据脱敏的提示词组合、最近请求/响应、媒体意图与工作流摘要、事件和耐久作业。检查器中的“测试媒体作业”会实际入队，因此不应在部署环境中开启该开关。
