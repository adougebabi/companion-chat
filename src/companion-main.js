...
async function openPersonaDetail(personaId) {
    try {
        ...
        const stateSource = state?.source?.kind === 'schedule' ? '来自已确认安排' : state?.source?.kind === 'routine' ? '来自日常作息' : state?.source?.kind === 'recovery' ? '服务恢复后同步' : state?.source ? '来自已记录生活事件' : '正在同步状态来源';
