...
const imageGenerationPolicyOptions = [
    ['ask', '始终询问'], ['always', '始终生成'], ['important', '重要时刻自动生成'], ['user_only', '只有我要求才生成'], ['autonomous', '摇光实例自行决定']
];
    ...
    $('#app').innerHTML = `
        <div class="app-frame">
            <aside class="rail" aria-label="主导航">
                <button class="brand" id="brand-button" aria-label="摇光首页">摇</button>
            ...
            <aside class="sidebar" id="sidebar">
                <header class="sidebar-header"><div><strong>摇光（Fluctlight）</strong><small>FLUCTLIGHT</small></div><button class="icon-button" id="create-button" aria-label="创建摇光实例" title="创建摇光实例">＋</button></header>
                ...
                <button class="new-persona" id="sidebar-create">＋ 创建一个摇光实例</button>
            </aside>
...
function renderSidebar() {
    $('#persona-list').innerHTML = appState.personas.length ? appState.personas.map(persona => `
        ...
        </button>`).join('') : '<div class="empty-list">还没有摇光实例</div>';
...
function renderContacts() {
    ...
    const visiblePersonas = selectedGroup
        ...
        : appState.personas;
    const empty = `<div class="contact-empty"><p>${selectedGroup ? `“${esc(selectedGroup.name)}”里还没有摇光实例。` : '还没有摇光实例。'}</p><button class="quiet" id="contacts-empty-create">创建一个摇光实例</button></div>`;
    $('#main-pane').innerHTML = `<section class="contacts-view"><header class="pane-header contacts-header"><button type="button" class="contacts-title" id="contacts-group-trigger" aria-label="选择联系人分组"><span class="header-copy"><h1>联系人</h1><p>${selectedGroup ? `${esc(selectedGroup.name)} · ${visiblePersonas.length} 位摇光实例` : '所有摇光实例的聊天'}</p></span></button><button class="text-icon" id="contacts-create" aria-label="创建摇光实例" title="创建摇光实例">＋</button></header><div class="contacts-stream">${visiblePersonas.length ? visiblePersonas.map(persona => `<button class="contact-row" data-contact-persona="${esc(persona.id)}">${avatar(persona)}<span class="persona-copy"><b>${esc(persona.name)}</b><small>${esc(persona.currentSituation || persona.role || '开始聊天')}</small></span>${persona.unreadCount ? `<em>${persona.unreadCount > 99 ? '99+' : persona.unreadCount}</em>` : ''}<i>›</i></button>`).join('') : empty}</div></section>`;
...
function renderSettingsPage() {
    $('#main-pane').innerHTML = `<section class="settings-view"><header class="pane-header"><div class="header-copy"><h1>设置</h1><p>管理应用与摇光实例</p></div></header><nav class="settings-menu"><button id="settings-model"><span>⚙</span><div><b>系统设置</b><small>模型、媒体与服务配置</small></div><i>›</i></button><button id="settings-create"><span>＋</span><div><b>创建摇光实例</b><small>开始一段新的关系</small></div><i>›</i></button><button id="settings-personas"><span>⌁</span><div><b>管理联系人</b><small>查看和切换已有摇光实例</small></div><i>›</i></button></nav></section>`;
...
function renderEmptyStart() {
    $('#main-pane').innerHTML = `<section class="empty-start"><div class="empty-mark">摇</div><h1>创建一个有自己生活的摇光实例</h1><p>从几句关于她是谁、如何生活的描述开始。设定完成后，她会在自己的世界里自然地上课、休息、见朋友，并把重要瞬间留在动态里。</p><button class="primary" id="empty-create">开始创建</button></section>`;
    ...
    $('#main-pane').innerHTML = `
        <section class="chat-view">
            <header class="pane-header chat-pane-header">
                ...
                <button class="chat-title" id="profile-button" aria-label="查看摇光实例详情">${avatar(persona, 'header-avatar')}<span><h1>${esc(persona.name)}</h1><p>${esc(persona.currentSituation || persona.role)}${persona.mood ? ` · ${esc(persona.mood)}` : ''}</p></span></button>
...
function mediaHtml(activity) {
    ...
    if (asset) return asset.kind === 'video'
        ...
        : `<img class="activity-media" src="${esc(asset.url)}" alt="${esc(activity.persona?.name || '摇光实例')}分享的图片" data-media-kind="image">`;
...
function renderActivities() {
    ...
    $('#main-pane').innerHTML = `<section class="activity-view"><header class="pane-header"><div class="header-copy"><h1>${momentsPersona ? `${esc(momentsPersona.name)}的动态` : '动态'}</h1><p>${momentsPersona ? '只属于她的生活瞬间' : '所有摇光实例的生活瞬间'}</p></div>${momentsPersona ? '<button class="quiet" id="all-activities">全部动态</button>' : ''}<button class="refresh-button" id="refresh-activity" aria-label="刷新动态" title="刷新动态">↻</button></header><div class="activity-stream" id="activity-stream">${activityItems.length ? activityItems.map(activityHtml).join('') : '<div class="activity-empty">还没有动态。摇光实例的日常和事件会自然地出现在这里。</div>'}${loadMore}</div></section>`;
...
function openGroupWizard(trigger = document.activeElement) {
    ...
    dialog.innerHTML = `<form class="persona-wizard group-wizard" id="group-form"><header><div><small>CONTACT GROUP</small><h2>创建分组</h2></div><button type="button" class="close-dialog" id="close-group" aria-label="关闭">×</button></header><div class="wizard-body"><p class="wizard-intro">把摇光实例按你想要的方式整理起来，之后可以在联系人页快速切换。</p><label>分组名称<input name="name" maxlength="60" required data-initial-focus placeholder="例如：学习伙伴"></label></div><footer class="wizard-footer"><button type="button" class="quiet" id="cancel-group">取消</button><button class="primary">创建分组</button></footer></form>`;
...
function openPersonaPicker() {
    const dialog = $('#persona-dialog');
    dialog.innerHTML = `<section class="detail-sheet persona-picker"><header><div><small>FLUCTLIGHT INSTANCES</small><h2>切换摇光实例</h2></div><button class="close-dialog" id="close-persona-picker" aria-label="关闭">×</button></header><div class="detail-scroll"><div class="persona-list picker-list">${appState.personas.length ? appState.personas.map(persona => `<button class="persona-row ${persona.id === activePersonaId ? 'selected' : ''}" data-picker-persona="${esc(persona.id)}">${avatar(persona)}<span class="persona-copy"><b>${esc(persona.name)}</b><small>${esc(persona.currentSituation || persona.role)}</small></span>${persona.unreadCount ? `<em>${persona.unreadCount > 99 ? '99+' : persona.unreadCount}</em>` : ''}</button>`).join('') : '<p class="muted">还没有摇光实例</p>'}</div><button class="new-persona" id="picker-create">＋ 创建一个摇光实例</button></div></section>`;
    showDialog(dialog);
...
async function openPersonaWizard() {
    ...
    const renderDescription = errorMessage => {
        dialog.innerHTML = `<form class="persona-wizard" id="persona-description-form"><header><div><small>FLUCTLIGHT INSTANCE</small><h2>描述一下她</h2></div><button type="button" class="close-dialog" id="close-interview" aria-label="关闭">×</button></header><div class="wizard-body"><p class="wizard-intro">描述你希望她是谁、什么性格、生活背景以及你们如何相处。自然地写几句就够了。</p>${errorMessage ? `<p class="wizard-error" role="alert">${esc(errorMessage)}</p>` : ''}<label>摇光实例描述<textarea name="description" rows="9" maxlength="6000" required data-initial-focus placeholder="例如：她叫林晚，是在读设计专业的学生，性格细腻慢热，喜欢摄影和旧书。她和室友住在学校附近，和我先从自然、尊重边界的朋友关系开始。">${esc(description)}</textarea></label></div><footer class="wizard-footer"><button type="button" class="quiet" id="cancel-interview">取消</button><button class="primary">分析并预览</button></footer></form>`;
    ...
    const renderPersonaPreview = errorMessage => {
        ...
        if (!preview) return renderDescription(errorMessage || '摇光实例分析结果不可用');
...
async function openPersonaDetail(personaId) {
    try {
        ...
        const inferredFields = Object.entries(blueprint?.provenance || {}).filter(([, source]) => source === 'inferred').map(([field]) => ({routine: '作息', interests: '兴趣', visualBaseline: '外观印象', supportingCast: '初始社交圈', foundation: '身份核心'})[field] || field);
...
async function rollbackEvolution(personaId, evolutionId) {
    if (!window.confirm('撤销这次关系层变化？身份核心和已有记忆不会被删除。')) return;
    try {
...
async function reviseFoundation(persona) {
    ...
    foundation = window.prompt('写下新的身份核心。系统会保留前一个版本。', foundation);
...
async function restoreFoundation(personaId, revisionId) {
    if (!window.confirm('恢复此身份核心版本？系统会保留当前版本，并把恢复结果记为一个新版本。')) return;
    try {
