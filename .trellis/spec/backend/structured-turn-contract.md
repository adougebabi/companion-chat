...
| Direct conversation requests continuation with an ACTION/mixed/unknown or more than two calls | `query_continuation_contract_invalid`; execute no dependent continuation. |
| Pure-query continuation query/result/provider phase is replayed | Reuse frozen invocation, completed result, and stable Provider request identity; never repeat committed work. |
...
- Continuation tests prove the generic execution-class gate, 1–2 call limit,
  no visible first response, no Tools/third call/mutation response, B-layout
  assistant-history compatibility, and phase/result replay.
