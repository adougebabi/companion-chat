...
- Session triggers are `initialization|wakeup`; initial and WakeUp triggers share the same Core helper. WakeUp queues only when no active canonical exists and reuses an active session.
...
- `media.comfyui.visual_identity_workflow` is an optional structured workflow map. Its `seed`/`character_sheet` variants are selected only from explicit concept fields; the legacy `workflow` remains the Scene Image fallback. `{{prompt}}` injection and provider/job persistence use the existing MediaWorkflow.
