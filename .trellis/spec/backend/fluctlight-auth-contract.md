...
- Trigger: first-run setup, login/logout, session resolution/revocation, browser mutation, browser boundary→Core request, Actor authorization, or local account recovery.
...
- Go Core owns account/session/authorization. The public browser boundary owns browser
...
The browser cookie contains an opaque random session token only. browser boundary→Core also carries a separate service credential/identity that cannot substitute for the Human session.
...
- browser boundary sets `HttpOnly`, `Secure`, and `SameSite=Lax` cookie flags in production and enforces CSRF token/origin policy for state-changing browser requests.
- browser boundary forwards the opaque session; Core resolves the Human Actor and authorizes every command/query/media grant. Browser-supplied Actor IDs never establish authority.
- browser boundary service identity and Human session are independently required/validated. Core is not host-exposed by default.
...
| browser boundary service credential invalid | Reject request independently of Human session. |
| Password reset succeeds | Rotate credential revision and revoke all sessions atomically. |
| Core/DB is unavailable | Do not create local browser boundary-only session or cached authorization fallback. |
...
- Bad: default admin/admin, JWT in localStorage, browser boundary trusting `actorId` from JSON, storing sessions only in Redis, or allowing anonymous mode on LAN.
...
- browser boundary cookie/CSRF tests for flags, origin/token validation, login/logout clearing, and no localStorage credential contract.
...
- Network/Compose tests proving only browser boundary is host-exposed by default.
...
return coreClient.command(command, {
  ...
  serviceIdentity: browserServiceCredential,
});
```
