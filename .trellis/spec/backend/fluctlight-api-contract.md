...
- Trigger: Node BFF calls Python Core, Python exposes a command/query, or incremental visible output crosses the Core→BFF process boundary.
- Python uses pinned 3.13, FastAPI, Pydantic v2, and Uvicorn. These are transport/composition tools, not domain dependencies.
- Browser contracts are owned by Node BFF and are distinct from this internal contract.
...
- Node Core client: generated from the checked OpenAPI artifact; hand-written duplicate DTOs are prohibited.
...
| Generated Node client differs from OpenAPI artifact | CI failure; regenerate and review both. |
| Stream sequence repeats/skips unexpectedly | Terminate bounded error, record correlation diagnostics, never silently reorder. |
| More than one terminal event | Contract failure; Node forwards only the first terminal and records violation. |
| Node/browser disconnects during realization | Propagate cancellation; suppress later writes; settle frozen action per workflow policy. |
...
- Good: OpenAPI generates a typed Node client, one turn streams ordered NDJSON, Node translates it, and disconnect cancels realization without reverting the frozen decision.
...
- End-to-end Node→Core streaming cancellation test with suppression of writes after disconnect.
