...
persona system declares persistent switching rules, Core runs a bounded,
tool-free `persistent_switch_assessment` after the Main candidate's visible
reply has been resolved. It decides `keep|switch` for the persistent profile
that the next turn will use; it never replaces the current reply, executes a
capability, or enters the takeover Judge. If no switching entry is declared,
the ordinary one-call Main path remains unchanged.
...
- Good: a persistent switch assessment runs after the Main candidate, selects a
  declared target profile, and the next turn is built from that profile's
  Working Persona while the current assistant message remains unchanged.
...
- Bad: use `memory.recall` name matching instead of execution metadata, permit
  ...
  a reply, or recursively re-arbitrate the post-cognition switch assessment. A bounded
...
- Persistent-switch tests assert one Main candidate followed by one tool-free
  assessment, winner-only settlement, next-turn profile selection, and no
  recursive switch decision.
