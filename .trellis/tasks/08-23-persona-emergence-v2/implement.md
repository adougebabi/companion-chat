...
3. 设计 appraisal、memory consolidation、self-model、agency 四组版本化候选 schema 与 revision/CAS 规则；self-model 的长期特征使用结构化特征主张，摘要和人格坐标均从主张派生。
4. 先实现 interaction facts 到 appraisal/affect 的最小闭环，再实现证据驱动的记忆沉淀。
5. 实现 self-model 的候选、冲突和自动沉淀流程，并将摘要接入 prompt/debug；提供用户查看、修改、删除和回滚入口，以 user-authored revision 保留审计链。
6. 实现 agency intention 的资格门控、冻结、投递和拒绝/延迟状态；允许 LLM 自然生成解释，同时持久化 redacted 原因摘要、证据引用和门控结果供专用入口查看。
