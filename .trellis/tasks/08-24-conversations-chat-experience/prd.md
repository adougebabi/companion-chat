# T06 Conversation 与聊天体验

## Goal

实现 Actor-aware Conversation/Participant/Message、历史/已读/投递顺序、Core/BFF/browser NDJSON、Vue chat/composer/attachment/error/cancellation 完整链路。

## Requirements

- 父任务与依赖门禁：T05 completed and merged。
- 实施范围：conversations module、Actor Participant/Message/read/delivery、Core/BFF/browser NDJSON、generated contracts、Vue chat/history/composer/attachments/error/abort。
...
- [ ] Functional contract、real-PostgreSQL、failure、security/redaction、browser aggregate 和 full-product validation 全部由 T12 重新执行；本 child 不为这些场景生成 acceptance 结果。
