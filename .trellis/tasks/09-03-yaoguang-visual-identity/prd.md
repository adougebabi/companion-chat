...
用户在身份设定中填写语义化的胸部尺寸（例如罩杯 A/B/C），Core 通过代码内专用 adapter（明确 if/else 映射）将其转换为 workflow 使用的胸部 LoRA weight。映射值（例如 A→-5、B→-3）是待验证的代码常量，并由 adapter/schema 版本标识；解析后的 weight 严格限制在 `-10..10`（含边界）。正式 visual identity 同时保存语义输入、解析后的 weight 和 adapter 版本；每轮渲染冻结当时的三者快照，Scene Image 和 Visual Identity workflow 都读取同一约束。映射缺失、越界、NaN、非数值或不支持的罩杯按可诊断配置错误/pending 处理，不静默猜测。
...
- 自动 regenerate 的最大 attempt 次数默认为 3；达到上限后停在 `awaiting_review`/`failed_max_attempts`，不无限消耗 Provider 或 ComfyUI 资源。
- 罩杯语义字段先支持常见罩杯字符串（至少 A/B/C/D，大小写和空白归一化）；不支持的值进入 `renderer_config_pending`，不做自定义值的隐式线性外推。
